---
description: Generate an implementation-ready REASONS Canvas prompt from SPDD analysis or business context
...
### 1. Create/Update [ComponentType] — `[path/to/file]` `(AC: AC-1, DE-2 if applicable)`
...
### 2. Add/Update Tests — `[path/to/test-file]` `(AC: AC-1, DE-2 if applicable)`
- **Cases**:
  - [happy path]
  - [edge case]
  - [failure case]
...
- Tag Operations and test headings with `(AC: AC-n)` or `(DE: DE-n)` when they implement or verify specific acceptance criteria or derived expectations.
...
```markdown
## Safeguards
1. **Functional constraints** `(AC: AC-n / DE-n if applicable)`: [specific functional boundaries]
2. **Data constraints** `(AC: AC-n / DE-n if applicable)`: [data validity, migration, compatibility, retention]
3. **Security constraints** `(AC: AC-n / DE-n if applicable)`: [auth, authorization, privacy, injection, exposure]
4. **Performance constraints** `(AC: AC-n / DE-n if applicable)`: [latency, complexity, throughput, resource limits]
5. **Integration constraints** `(AC: AC-n / DE-n if applicable)`: [API contracts, backward compatibility, external dependencies]
6. **Error-handling constraints** `(AC: AC-n / DE-n if applicable)`: [classified errors, no sensitive leakage, retry/idempotency]
7. **Testing constraints** `(AC: AC-n / DE-n if applicable)`: [required coverage, regression scenarios, manual checks]
8. **Rollout constraints** `(AC: AC-n / DE-n if applicable)`: [migration, feature flag, deployment, rollback]
```
...
- Tag Safeguards with `(AC: AC-n)` or `(DE: DE-n)` when they enforce specific acceptance criteria or derived expectations.
...
The saved prompt must include only the generated REASONS Canvas content. Do **not** include:
...
- **collision handling**: If the exact target file already exists, append `-v2`, `-v3`, and so on rather than overwriting. If a file with the same `{TICKET}-...-{description}.md` prefix already exists for the same ticket and requirement, ask the user whether to overwrite it (typical for in-session iteration), create a versioned copy, or abort. Default to asking rather than accumulating stale versions silently.
...
The REASONS Canvas structured prompt is ready. Would you like to invoke the implementation workflow next?

This prompt does not implement code. If the user confirms, explain that implementation should be handled by a separate implementation prompt such as `/spdd-generate` that reads the saved canvas and enforces its Operations, Norms, and Safeguards. If no such prompt exists yet, acknowledge that the local SPDD loop is not closed and ask the user how they would like to proceed. pi does not auto-chain prompt templates; the next workflow must be invoked explicitly in the conversation.

## Guardrails

- Do not proceed without input context.
- Do not implement code in this prompt. This prompt only creates the saved REASONS Canvas artifact and asks for confirmation for a later implementation workflow.
